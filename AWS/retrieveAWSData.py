...
import json
'''
Variation of CapitalBikeAPI.py that is meant to retrieve data saved on our AWS instance.
Each row in the table will be turned into an insert statement to be saved into a text file.
This text can later be used to re-create the database.
'''
...
'''
Pulling from the database and turning each row into an insert statement.
These can be used in the future repopulate a database.
'''
cur.execute("select * from stations")
rows = cur.fetchall()
with open('stations.txt', 'a') as file:
    for row in rows:
        s = ('INSERT INTO stations VALUES (' + str(row[0]) + ',' + str(row[1]) + ',' + str(row[2]) + ',' + str(row[3]) + ',' + str(row[4])+ ',' + str(row[5])+ ',' + str(row[6])+ ')')
        file.write(s + '\n')
#stationID,bikesAvailable,ebikesAvailable,bikesDisabled,docksAvailable,docksDisabled,isInstalled,isRenting, isReturning, date
cur.execute("select * from station_status")
rows = cur.fetchall()

with open('stationStatus.txt', 'a') as file:
    for row in rows:
        s = ('INSERT INTO station_status VALUES (' + str(row[0]) + ',' + str(row[1]) + ',' + str(row[2]) + ',' + str(row[3]) + ',' + str(row[4])+ ',' + str(row[5])+ ',' + str(row[6]) + str(row[7])+ ',' +  str(row[8])+ ',' + str(row[9])+ ')')
        file.write(s + '\n')
